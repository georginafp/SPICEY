...
For each feature $x$ (e.g., gene or regulatory region), the activity or expression scores across $N$ cell types are normalized into a probability distribution. This allows measuring how specific or broadly distributed a feature's activity is across cell types. 
Lower entropy values indicate higher cell-type specificity, while higher entropy reflects more ubiquitous expression or accessibility. These entropy-based indices are computed as follows:
...
```{r quick-example, fig.width=2, fig.height=3}
...
1.  **Filters and scales the input**: Extracts the SPICEY scores for the selected specificity index (`RETSI`, `GETSI`, or a combined SPICEY score).
2.  **Selects top genes**: For each cell type, identifies the top `n` genes based on their SPICEY score ranks.
3.  **Generates a heatmap**: Creates a gene-by-cell-type heatmap. Genes are ordered by their highest-scoring cell type and their SPICEY score magnitude.

Depending on the `spicey_measure` and `combined_score` arguments, the function adapts the output:

-   **Single-modality visualization:**
    -   `spicey_measure = "RETSI"`: Plots scored RETSI values, where RETSI reflects cell-type-specific accessibility of regulatory regions linked to genes.
    -   `spicey_measure = "GETSI"`: Plots scored GETSI values based on gene expression.
-   **Dual-modality (SPICEY) visualization:**
    -   `spicey_measure = "SPICEY"` and `combined_score = FALSE`: Displays two separate heatmaps -one for RETSI, one for GETSI - enabling side-by-side comparison.
    -   `spicey_measure = "SPICEY"` and `combined_score = TRUE`: RETSI and GETSI are independently scored and averaged per gene-cell-type pair. A single unified heatmap is generated to highlight shared specificity across both data types.
...
-   `combined_score`: Logical. If `TRUE` and `spicey_measure = "SPICEY"`, computes a combined mean score across RETSI and GETSI (default: `FALSE`).

```{r spicey-heatmap-sep, message=FALSE, warning=FALSE, fig.width=2, fig.height=3}
...
```{r spicey-heatmap-pair, message=FALSE, warning=FALSE, fig.width=3.5, fig.height=4.5}
spicey_heatmap(spicey_coacc$linked,
  spicey_measure = "SPICEY",
...
```{r spicey-heatmap-comb, message=FALSE, warning=FALSE, fig.width=2, fig.height=3}
