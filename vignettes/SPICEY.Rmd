...
- **Required Columns (per element):**
  ...
  - `p_val` — Raw p-value from the differential expression test.
  - `p_val_adj` — Adjusted p-value from the differential test.
...
2. (**Optional**) Annotates regulatory elements to target genes using either:
   - Nearest gene transcription start site (TSS) ("nearest")
   - Co-accessibility links ("coaccessibility"), e.g., derived from Cicero
...
| `p_val`         | Raw p-value from differential testing.                                                          |
...
| `gene_id`       | Gene identifier using official gene symbols (e.g., `GAPDH`).                                    |
...
| `p_val_ATAC`    | Raw p-value from differential accessibility testing.                                            |
...
| `gene_id`       | Gene identifier (official gene symbol).                                                         |
| `p_val_adj_RNA` | Adjusted p-value from differential expression testing.                                          |
| `p_val_RNA`     | Raw p-value from differential expression testing.                                               |
